...
window.iconphoto(True, tk.PhotoImage(file='files/images/Tanks_icon.png'))

background = tk.PhotoImage(file='files/images/Background_log.png')
canvas = tk.Canvas(window, width=450, height=250)
canvas.create_image(225, 125, anchor='center', image=background)
canvas.create_image(225, 125, anchor='center')
...
def registration():
    ...
    reglabel = tk.Label(regwindow, text='Регистрация в игре "Tanks"', font='Calibri 15 bold', bg='black', fg='orange',
                        relief='ridge', borderwidth=7)
    reglabel.place(x=100, y=5)

    regloglabel = tk.Label(regwindow, text='Придумайте логин:', font='Calibri 12 bold', bg='black', fg='orange',
                           relief='ridge', borderwidth=5)
    regloglabel.place(x=155, y=50)

    reglogentry = tk.Entry(regwindow, text='', font='Calibri 12 bold italic', width=30, relief='ridge', borderwidth=5,
                           bg='gray20', fg='orange')
    reglogentry.place(x=102, y=85)

    regpasslabel = tk.Label(regwindow, text='Придумайте пароль:', font='Calibri 12 bold', bg='black', fg='orange',
                            relief='ridge', borderwidth=5)
    regpasslabel.place(x=150, y=125)

    regpassentry = tk.Entry(regwindow, text='', show='*', font='Calibri 12 bold italic', width=30, relief='ridge',
                            borderwidth=5, bg='gray20', fg='orange')
    regpassentry.place(x=102, y=160)

    regenterbutton = tk.Button(regwindow, text='Создать аккаунт!', font='Calibri 15 bold', relief='ridge',
                               borderwidth=5, bg='orange', fg='black', command=lambda: save())
...
def enter():
    x = []
    with open('files/Database.txt', 'rb') as fr:
        try:
            while True:
    ...
    if check is not None:
        ...
        process = subprocess.Popen(f"python start_game.py {check['type']}",
